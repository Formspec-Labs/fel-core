...
.PHONY: all help build test test-full test-differential test-all emit-fixtures fuzz-extract fuzz-setup fuzz-coverage fuzz-all seed-fuzz clean
...
help:
	...
	@echo "  make test-differential  — run cross-runtime oracle (needs formspec-py)"
...
test-differential:
	$(CARGO) test $(CARGO_FLAGS_PROPTEST_FEATURES) --test differential_oracle -- --ignored --test-threads=1
